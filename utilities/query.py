...
MODEL_PATH = "/workspace/search_with_machine_learning_course/predict_query_ngram.bin"

class Model:
    def __init__(self):
        self.query_model = fasttext.load_model(MODEL_PATH)

    def predict_label(self, query):
        category_list = []
        total = 0

        categories = self.query_model.predict(query, k=5)

        for (label, score) in zip(categories[0], categories[1]):
